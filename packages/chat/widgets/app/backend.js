...
`),d.end()),await P(r,oe,R)}catch(e){console.error("Streaming error:",e),await H(d,e.message),d.end()}void await 0}else if(G){var we=T;var O=S;var L=o;var _e=c;var F=n;var ve=t;var Se=g;var Te=f;let a=null,r=[];we.forEach((e,t)=>{t!==we.length-1&&({role:t,message:e}=e,""!==e)&&(t===SYSTEM_ROLE?a=e:r.push({role:t,content:e}))});var U={messages:U=populateMessages(L,r),model:F,temperature:_e,max_tokens:ve,stream:!0};a&&(F=populateMessages(L,[{content:a}]),U.system=F[0].content);try{l.setTimeout(3e4);var $=Date.now();let e=!1;l.on("close",()=>{console.log("Client closed connection"),e=!0});var b,x,Ae,Ne=new Anthropic({apiKey:Se}),k=(console.log(`Starting stream ${Te}...`),{startedAt:(new Date).getTime()}),Ie=await Ne.messages.create(U);O.createdAt=Date.now(),O.payload=U;let t=!1,a="";for await(b of Ie){if(O.updatedAt=Date.now(),e)break;if("content_block_delta"===b.type&&(x=b.delta.text||"")&&(a+=x,Ae={content:x,done:!1},d.write(`id: ${$}
event: message
